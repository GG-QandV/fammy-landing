...
- Git branch изоляция: Ветка `landing-v3` (архитектура V3) была слита в `main`. `main` теперь является основной боевой веткой (Production).
- Деплой на продакшен (через Coolify) выполняется автоматически при пуше в ветку `main`. Ветка `main-legacy` хранит старую V2 версию.
## Известные проблемы
